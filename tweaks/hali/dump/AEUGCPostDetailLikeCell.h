//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "UITableViewCell.h"

@class NSMutableArray, UIImageView, UILabel, UIView;

@interface AEUGCPostDetailLikeCell : UITableViewCell
{
    _Bool _didSetupConstraints;	// 8 = 0x8
    UIImageView *_splitview;	// 16 = 0x10
    long long _likeCount;	// 24 = 0x18
    UIImageView *_likeview;	// 32 = 0x20
    UILabel *_likeLabel;	// 40 = 0x28
    UIImageView *_headerview;	// 48 = 0x30
    UIView *_indicatorview;	// 56 = 0x38
    NSMutableArray *_likeViewArr;	// 64 = 0x40
    UILabel *_allLabel;	// 72 = 0x48
    UIView *_topLineView;	// 80 = 0x50
}

@property(retain, nonatomic) UIView *topLineView; // @synthesize topLineView=_topLineView;
@property(nonatomic) _Bool didSetupConstraints; // @synthesize didSetupConstraints=_didSetupConstraints;
@property(retain, nonatomic) UILabel *allLabel; // @synthesize allLabel=_allLabel;
@property(retain, nonatomic) NSMutableArray *likeViewArr; // @synthesize likeViewArr=_likeViewArr;
@property(retain, nonatomic) UIView *indicatorview; // @synthesize indicatorview=_indicatorview;
@property(retain, nonatomic) UIImageView *headerview; // @synthesize headerview=_headerview;
@property(retain, nonatomic) UILabel *likeLabel; // @synthesize likeLabel=_likeLabel;
@property(retain, nonatomic) UIImageView *likeview; // @synthesize likeview=_likeview;
@property(nonatomic) long long likeCount; // @synthesize likeCount=_likeCount;
@property(retain, nonatomic) UIImageView *splitview; // @synthesize splitview=_splitview;
- (void).cxx_destruct;	// IMP=0x00000001006ea94c
- (void)updateConstraints;	// IMP=0x00000001006e9b08
- (void)bindData:(id)arg1;	// IMP=0x00000001006e92d0
- (void)layoutSubviews;	// IMP=0x00000001006e9228
- (id)initWithStyle:(long long)arg1 reuseIdentifier:(id)arg2;	// IMP=0x00000001006e9044

@end

