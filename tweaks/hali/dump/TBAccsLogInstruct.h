//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "TBJSONModel.h"

@class NSDictionary, NSString;

@interface TBAccsLogInstruct : TBJSONModel
{
    _Bool _success;	// 8 = 0x8
    NSString *_appKey;	// 16 = 0x10
    NSString *_deviceId;	// 24 = 0x18
    long long _command;	// 32 = 0x20
    NSDictionary *_data;	// 40 = 0x28
    long long _serialNumber;	// 48 = 0x30
    long long _taskId;	// 56 = 0x38
    NSString *_user;	// 64 = 0x40
    long long _expireTime;	// 72 = 0x48
    NSString *_errCode;	// 80 = 0x50
    NSString *_errMsg;	// 88 = 0x58
}

+ (id)getObjectInternal:(id)arg1;	// IMP=0x0000000101a30cc8
+ (id)getObjectData:(id)arg1;	// IMP=0x0000000101a30b20
+ (id)getJSON:(id)arg1 options:(unsigned long long)arg2 error:(id *)arg3;	// IMP=0x0000000101a30aa4
@property(retain, nonatomic) NSString *errMsg; // @synthesize errMsg=_errMsg;
@property(retain, nonatomic) NSString *errCode; // @synthesize errCode=_errCode;
@property(nonatomic) long long expireTime; // @synthesize expireTime=_expireTime;
@property(nonatomic) _Bool success; // @synthesize success=_success;
@property(retain, nonatomic) NSString *user; // @synthesize user=_user;
@property(nonatomic) long long taskId; // @synthesize taskId=_taskId;
@property(nonatomic) long long serialNumber; // @synthesize serialNumber=_serialNumber;
@property(retain, nonatomic) NSDictionary *data; // @synthesize data=_data;
@property(nonatomic) long long command; // @synthesize command=_command;
@property(retain, nonatomic) NSString *deviceId; // @synthesize deviceId=_deviceId;
@property(retain, nonatomic) NSString *appKey; // @synthesize appKey=_appKey;
- (void).cxx_destruct;	// IMP=0x0000000101a3131c

@end

